...
def handle_requirement_selection(app, event, sub_point_text=None):
    ...
    if req_code in app.requirements_data:
        ...
        full_designation = req_data.get('full_designation', req_code)
        display_designation = full_designation
        if "Disclosure Requirement" in full_designation:
            display_designation = full_designation.replace("Disclosure Requirement", translate("disclosure_requirement"))
        
        app.text_display.insert("end", f"{display_designation}\n", "h1")
        
        # Show separator line after full_designation
        app.text_display.insert("end", "_" * 50 + "\n")

        app.text_display.insert("end", f"{text_to_display}\n\n")

        # Only display matches if a sub-point is selected
        if sub_point_text:
